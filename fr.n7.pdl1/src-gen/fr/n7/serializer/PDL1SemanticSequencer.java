...
public class PDL1SemanticSequencer extends AbstractDelegatingSemanticSequencer {
	...
	protected void sequence_Resource(ISerializationContext context, Resource semanticObject) {
		if (errorAcceptor != null) {
			...
			if (transientValues.isValueTransient(semanticObject, PDL1Package.Literals.RESOURCE__QUANTITY) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, PDL1Package.Literals.RESOURCE__QUANTITY));
		}
		SequenceFeeder feeder = createSequencerFeeder(context, semanticObject);
	 ...
	 *
	 * Constraint:
	 *     (name=ID (usedResources+=[Resource|ID] usedResources+=[Resource|ID]*)?)
	 * </pre>
	 */
