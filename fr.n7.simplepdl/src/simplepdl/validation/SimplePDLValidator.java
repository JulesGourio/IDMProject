...
public class SimplePDLValidator extends SimplepdlSwitch<Boolean> {
	...
	public Boolean caseWorkDefinition(WorkDefinition object) {
		...
		this.result.recordIfFailed(
			object.eContainer().eContents().stream()
	...
	public Boolean caseWorkSequence(WorkSequence object) {
		...
		// DEBUT - Ajout pour l'exercice 3.2
		// On récupère le process parent pour trouver les autres WorkSequence
		...
		this.result.recordIfFailed(
			...
			"' entre l'activité '" + object.getPredecessor().getName() +
			"' et '" + object.getSuccessor().getName() + "'.");
		// FIN - Ajout pour l'exercice 3.2
	...
	public Boolean caseGuidance(Guidance object) {
		this.result.recordIfFailed(
				...
				object, 
				"La guidance ne contient pas de texte.");
		return null;
	}
